...
const VerifyLogin = ({ isOpen, onClose, payload }) => {
  ...
  const { mutate: verify, data, isLoading, error } = useVerifyLogIn();
  ...
  useEffect(() => {
    if (!!data && data?.status === 200) {
      ...
      const walletData = JSON.stringify(data?.data?.wallet);
      localStorage.clear();
      dispatch({ type: userActions.LOGIN, payload: data?.data?.user });
      localStorage.setItem(token, data?.data?.user.access_token);
